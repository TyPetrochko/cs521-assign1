 ...
 *
 * This file is a vector triad benchmark that measures performance
 * in MFlop/sec of 
 *  
 *    a[i] = b[i] + c[i] * d[i]
 *
 * for various vector lengths. Here, N represents the vector
 * length exponent.
 */
#include <stdio.h>
#include <stdlib.h>
...
void benchmark(int N){
  ...
  init_rand(a, size); // why not; doesn't hurt
  init_rand(b, size);
  init_rand(c, size);
  init_rand(d, size);
